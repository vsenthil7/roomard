...
// bcryptjs is a CommonJS module without proper named-export ESM interop.
// Default import + destructure is the only pattern that works under Node
// 20's native ESM loader. Named imports (`import { compare } from 'bcryptjs'`)
// look fine in TypeScript and pass in vitest (esbuild interop), but crash
// at runtime with "Named export 'compare' not found".
...
export class AuthService {
  ...
  private async verifyMfaToken(token: string): Promise<{ userId: string; tenantId: string }> {
    // jwtVerify is imported at top-level — previously this used a dynamic
    // `await import('jose')` which (a) cost an extra microtask per MFA verify
    // on a hot security path, and (b) implied lazy-load semantics the rest of
    // the file already contradicts by eagerly importing SignJWT.
    const secret = new TextEncoder().encode(this.cfg.jwtSecret);
    try {
