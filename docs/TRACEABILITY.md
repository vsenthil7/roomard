...
**Last updated:** 2026-05-21 22:02 BST (CP-87)
**Live source of truth:** `origin/main` on https://github.com/vsenthil7/roomard

**Total tests:** 356 passing, 0 failing (workspace unit suites); **12 real-DB integration tests** (guest 4, tenant 5, capture 3) guarding the G-41/42/43 schema-drift class — now **idempotent + re-runnable** (verified twice consecutively 12/12) via `scripts/run-integration-tests.bat`; plus the pre-existing 12 db-package integration tests

> **Demo video status (CP-87, honest):** the committed demo video remains the morning verdict-edition (`demo/roomard-walkthrough-20260521_092058-main.mp4`, verify-a 24/24 + verify-b 13/13). A Stage-5 “regression test layer” demo iteration was built and verified (verify-a 24/24, integration 12/12 on screen) but **deliberately NOT promoted as the final video** — user feedback (22:00) is that the demo must be rebuilt as an enterprise product story (new-hotel onboarding → first data entry → each use case shown as expected-screen-flow-then-live-test), not an engineering/test-internals tour. That rebuild is the next work item; the Stage-5 spec is committed as an interim, the draft MP4s are parked in `_session/draft_mp4s/` (not committed).

---

## Session timeline (CP-1 → CP-87)
...
### Commits landed (newest → oldest, 87 total since session start)

| Commit | CP | Type | Summary | Verified |
|---|---|---|---|---|
| (this) | CP-87 | [TEST] | **Integration tests made idempotent + a runnable harness + an interim Stage-5 demo iteration.** (1) Fixed a real test-quality bug: the guest integration test seeded a stay with a FIXED `pms_booking_id`, which tripped the `(tenant, pms_provider, pms_booking_id)` unique constraint on the SECOND run — so it passed once then failed for anyone re-running it. Now uses a per-run booking id derived from the fresh guest id; **verified by running the full suite twice consecutively — 12/12 both times**. (2) Added `scripts/run-integration-tests.bat` so anyone can run all 12 with verbose, per-service narration. (3) Added `_session/run-integration-and-report.mjs` (orchestration) that runs the 12 integration tests against the live DB and emits a JSON result. (4) Added an **interim Stage-5** to `full-walkthrough.spec.ts` that reads that JSON and shows the real 12/12 result on screen, with a matching verify-b rubric row. **NOTE:** the Stage-5 demo approach is interim only — per user feedback it is being replaced by a proper enterprise product-story demo (see header note); the draft MP4s were NOT committed. | ✅ integration 12/12 ×2 (idempotent); Stage-5 iteration verify-a 24/24 | **Real-database integration tests — the schema-drift regression layer.** The unit suites were green throughout the G-39..G-43 cascade because they drive handlers with a fake `pg` client that returns hand-written rows and never validates column names against the real schema (the fake-pool blind spot, admitted in its own source comment). This adds **12 integration tests** that run the ACTUAL exported production code against a REAL Postgres DB, so column drift raises `42703` and the test fails — which the unit suite cannot do: `services/guest/tests/integration/service-db.test.ts` (4 tests, G-41 — real `GuestRepo.getPreferences`/`getHistory`/`analyseComplaintTrajectory`), `services/tenant/tests/integration/server-db.test.ts` (5 tests, G-42 — real `/v1/tenant` + `/v1/properties` list/by-id/insert via `app.inject()` on a real pool), `services/capture/tests/integration/server-db.test.ts` (3 tests, G-43 — real `GET /v1/captures/:id` + unknown-id→404). Each suite has a meta-assertion proving the phantom columns genuinely don't exist (the guard has teeth) and every `it()` explains what it proves + which bug it guards. Gated on `DATABASE_URL`: skip cleanly + announce when unset (default `pnpm test` stays green with no DB), run for real with the dev container Postgres. Added `@roomard/test-utils` to capture-svc devDeps. Documented as §17 in `docs/roomard_teststrategy.md` (v1.1), including the `pnpm -r test` web-e2e abort caveat. Writing these surfaced real schema facts the mocks hid (guests `email_lower` not `email`; polarity `likes`; `stays` needs `pms_booking_id`+`pms_provider`; address_json keys are snake_case). | ✅ guest 4/4, tenant 5/5, capture 3/3; skip-clean verified |
