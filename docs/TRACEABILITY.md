...
**Last updated:** 2026-05-21 23:20 BST (CP-88)
**Live source of truth:** `origin/main` on https://github.com/vsenthil7/roomard

**Total tests:** 356 passing, 0 failing (workspace unit suites; web 39/39 incl. the new onboarding route's typecheck); **12 real-DB integration tests** (guest 4, tenant 5, capture 3) idempotent + re-runnable via `scripts/run-integration-tests.bat`

> **Demo video status (CP-88):** rebuilding the demo as an enterprise product story — a brand-new hotel starting from zero, onboarding through the real UI, then each use case shown as expected-screen-flow-then-live-test. CP-88 lands the prerequisite: a real **`/onboarding` UI wizard** (create property → add guests → generate brief, all via live APIs) so “how a new hotel first feeds in data” is a genuine on-screen flow, plus **G-44** fixed so the brief actually populates. The committed demo MP4 remains the morning verdict-edition until the new product-story recording is verified.

---

## Session timeline (CP-1 → CP-88)
...
### Commits landed (newest → oldest, 88 total since session start)

| Commit | CP | Type | Summary | Verified |
|---|---|---|---|---|
| (this) | CP-88 | [FEAT] | **New `/onboarding` UI wizard (the “new hotel starts from zero” flow) + G-44 brief fix — groundwork for the enterprise product-story demo.** (1) `apps/web/src/routes/onboarding.tsx`: a real 3-step setup wizard for a brand-new tenant — Step 1 create property (`POST /v1/properties`), Step 2 add first guests (`POST /v1/guests`), Step 3 generate first brief (`POST /v1/briefs/generate`), then a “your hotel is live” summary. Each step calls a live, existing API (no mocks); matches the app design system (teal `roomard-*`, `card`/`form-input`/`btn-primary`) and carries `data-testid`s for the demo to drive. Registered in `routeTree.ts`; “Set up” added to the header nav. This makes “how a new hotel first feeds in data” an authentic on-screen flow rather than a seed script. (2) **G-44** (`services/brief/src/pipeline.ts`): the brief’s arrival query filtered on `status IN ('booked','expected','checked_in')` but the real `stay_status` enum is `confirmed,checked_in,checked_out,no_show,cancelled` — so a normal **confirmed** booking was invisible to the brief (blank home screen); it also used the `title`/`occurred_at` phantom columns (same drift as G-41) which would 500 on any arriving guest with a recent issue. Fixed to `confirmed`/`checked_in` + `summary AS title`/`raised_at`. Confirmed `prep-cards.ts` already correct. | ✅ web tsc clean + 39/39 web tests; both images rebuilt + healthy; `GET /onboarding → 200` |
