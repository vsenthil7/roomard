...
export class RoomardPool {
  ...
  constructor(input: pg.Pool | DbConfig) {
    // Accept either a constructed pg.Pool (test scenarios with mocks)
    // or a DbConfig (production / service boot). Keeps call sites stable
    // across the workspace.
    if (input instanceof Pool) {
      this.pool = input;
    } else {
      const wrapped = createPool(input);
      this.pool = wrapped.raw;
    }
  }
